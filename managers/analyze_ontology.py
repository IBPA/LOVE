...
def getPairsfromDict(candidate_dict):
    pairs = []
...
def check_lineage(gtp, pp, graph):
    ...
    paths = find_all_paths(graph, gtp, pp)
...
class AnalyseOntology:
    def __init__(self, config_filepath):
        ...
        self.foodonpairs = self.configparser.getstr('foodonpairs')
        self.gt_entitymapping = self.configparser.getstr('gt_entitymapping')
        # Load data from files
        ground_truth = load_pkl(self.gt_entitymapping)
        self.gtDF = getPairsfromDict(ground_truth)
...
if __name__ == '__main__':
    ao = AnalyseOntology('../config/analyse_ontology_results.ini')
